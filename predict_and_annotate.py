...
def compute_subtraction(template_path, test_path, output_path):
    """Generate subtraction map with black background and colored defects."""
    ...
    black_background = np.zeros_like(test)

    # Apply the heatmap only to the defect areas
    heat = cv2.applyColorMap(mask, cv2.COLORMAP_JET)
    colored_defects = cv2.bitwise_and(heat, heat, mask=mask)

    # Combine the black background with the colored defects
    result = cv2.addWeighted(black_background, 1, colored_defects, 1, 0)

    cv2.imwrite(output_path, result)
    print(f"✓ Subtraction image saved with black background: {output_path}")
    return True
